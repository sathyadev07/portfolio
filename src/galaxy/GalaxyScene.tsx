...
/* Cool Stuff ON. Every destination is a real object at a real coordinate inside
 * the galaxy, mounted once and projected from its own world transform. There is
...
export default function GalaxyScene({paused,onOpenDetail,onExit}:{paused:boolean;onOpenDetail:OpenDetail;onExit:()=>void}){
 ...
 return <div className="galaxy-mode">
  <GalaxyCanvas reducedMotion={false}>
   <group ref={frame}>
    <GalaxyCloud reducedMotion={reduced}/>
  ...
  <main ref={root} id="main-content" className="journey-stage" data-od-id="journey-stage" aria-label="Galaxy journey" tabIndex={-1}>
   <p className="sr-only">Scroll or swipe to fly through the galaxy. Arrow up and down also move through the journey. Activate an experience or project to read its details. Escape returns to the portfolio.</p>
