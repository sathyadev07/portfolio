...
const srcSetFor=(file:string,width:number)=>[512,1024]
 ...
 .join(', ');
export default function MediaGallery({id}:{id:string}){const root=useRef<HTMLDivElement>(null);useReveal(root);const gallery=GALLERIES[id as keyof typeof GALLERIES]??[];return gallery.length?<><h3 className="portfolio-detail-sub">Build media</h3><div ref={root} className="portfolio-gallery">{gallery.map(([file,caption],index)=>{const dimensions=MEDIA[file as keyof typeof MEDIA];const set=dimensions?srcSetFor(file,dimensions.width):'';return <RevealOnScroll key={file} step={index}><figure><a href={asset(`assets/images/${file}`)} target="_blank" rel="noopener noreferrer" aria-label={`Open full image: ${caption}`}><GlowMedia><img className="od-media" src={asset(`assets/images/${file}`)} {...(set?{srcSet:set,sizes:SIZES}:{})} width={dimensions?.width} height={dimensions?.height} alt={caption} loading="lazy" decoding="async"/></GlowMedia></a><figcaption>{caption}</figcaption></figure></RevealOnScroll>})}</div></>:null;}
