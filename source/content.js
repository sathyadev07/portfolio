...
(function () {
  ...
  const COPY = {
    ...
    about: "Hello! My depth is in mechanical design, structural analysis, and design-for-manufacturing. I've machined drivetrain components for Formula SAE Electric and designed production tooling for rotorcraft at Sikorsky, and what I care about is the full path from geometry and tolerancing to a part that comes off a machine and works. From that base I've pushed outward into the systems around the hardware: sensor fusion and closed-loop control, analog circuit design, and Python simulations for feasibility and trade studies. I got into engineering to work on problems that genuinely help people and the planet, and the more of the stack I understand, the more useful I can be on them.",
  ...
  function logoMark(logoType, context) {
    ...
    return el('span', { class: 'logo-mark', 'data-logo': logoType, 'data-logo-context': context }, [
      el('img', {
        ...
        width: logo.width,
        height: logo.height,
        /* Static mode puts every panel in the stage at once, so without this
           both company logos were fetched for a visitor who never reached
           either employer. preparePrint() forces eager on the print copy. */
        loading: 'lazy',
        decoding: 'async'
      })
    ]);
  ...
  /* The two documents a visitor can leave with. They live in About now, along
     with the rest of the supporting content; only their placement moved, so
     the ids the controller binds its click handlers to are unchanged. */
  function documentActions() {
    return el('div', { class: 'actions od-cluster', 'data-od-id': 'about-actions' }, [
      el('a', {
  ...
  /* The opening frame is the name over the galaxy and nothing else. It carries
     no card: `panel-bare` drops the panel treatment so the lettering sits
     directly on the sky, and the shadow in styles.css is what keeps it legible
     there. */
  function heroPanel() {
    return el('div', { class: 'panel-inner panel-bare', 'data-od-id': 'hero-panel' }, [
      el('h1', { class: 'display name', id: 'panel-title-hero', 'data-od-id': 'hero-name', text: COPY.name })
    ]);
  }

  /* About now carries everything the hero used to: who this is, the summary,
     the record, and the documents. Density comes from grouping and grid rather
     than from shrinking type — running the facts as a multi-column strip
     instead of a five-row list is most of the height saved. */
  function aboutPanel() {
    return el('div', { class: 'panel-inner about-panel', 'data-od-id': 'about-panel' }, [
      ...
      el('dl', { class: 'facts facts-strip', 'data-od-id': 'hero-facts' }, COPY.facts.reduce(function (acc, fact) {
